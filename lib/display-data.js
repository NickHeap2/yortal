  ...
  for (let cf of cfs) {
    ...
    if (cf.Origins.Quantity > 0) {
      ...
      for (let origin of cf.Origins.Items) {
        ...
        if (cacheBehaviour) {
          console.log(`    Caching Behaviour MinTTL=[${cacheBehaviour.MinTTL}], DefaultTTL=[${cacheBehaviour.DefaultTTL}], MaxTTL=${cacheBehaviour.MaxTTL}`.blue)

          if (cacheBehaviour.LambdaFunctionAssociations.Quantity > 0) {
            console.log('    Associated Lambda Functions:'.cyan)
            for (const lambdaFunction of cacheBehaviour.LambdaFunctionAssociations.Items.sort()) {
          ...
          console.log(`    No caching behaviour found`.yellow)
        }
...
function displayCloudFronts(cfs) {
  for (let cf of cfs) {
    ...
    for (let origin of cf.Origins.Items) {
      ...
      const cacheBehaviour = cf.CacheBehaviors.Items.find((behaviour) => behaviour.TargetOriginId === origin.Id)
      if (cacheBehaviour) {
        console.log(`    Caching Behaviour MinTTL=[${cacheBehaviour.MinTTL}], DefaultTTL=[${cacheBehaviour.DefaultTTL}], MaxTTL=${cacheBehaviour.MaxTTL}`)
      } else {
        console.log(``)
      }
    }
