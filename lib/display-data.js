...
function displayFunctions(functions) {
  for (const theFunction of functions) {
    console.log(`FunctionArn[${theFunction.FunctionArn}] Timeout=[${theFunction.Timeout}] Code Size=[${theFunction.CodeSize}] Memory Size=[${theFunction.MemorySize}]`.yellow)
  }
}
  ...
  if (cacheBehaviour.LambdaFunctionAssociations.Quantity > 0) {
    ...
    for (const lambdaFunction of cacheBehaviour.LambdaFunctionAssociations.Items.sort((a, b) => a.EventType.localeCompare(b.EventType))) {
      ...
      if (theFunction) {
        ...
        console.log(`        Failed to find function!`.yellow)
      }
    }
  }
...
function displayFunction(theFunction) {
  // console.log(`        Timeout=[${theFunction.Timeout}] Code Size=[${theFunction.CodeSize}] Memory Size=[${theFunction.MemorySize}]`.yellow)
  // console.log(`          Timeout=[${theFunction.Timeout}] Code Size=[${theFunction.CodeSize}] Memory Size=[${theFunction.MemorySize}]`.yellow)
  console.log(`          Timeout=[${theFunction.Timeout}] Code Size=[${theFunction.CodeSize}] Memory Size=[${theFunction.MemorySize}]`.yellow)
