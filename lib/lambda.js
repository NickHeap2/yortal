import { LambdaClient, ListFunctionsCommand, ListProvisionedConcurrencyConfigsCommand, GetAliasCommand } from '@aws-sdk/client-lambda'
...
export async function getAliases(credentials, functions, region) {
  const client = new LambdaClient({ region, credentials });

  const params = {}
  const command = new ListAliasesCommand(params)

  const bar = startProgressBar(functions, '      Fetching Alises')
  for (const theFunction of functions) {
    command.input = {
      FunctionName: theFunction.FunctionArn,
      MaxItems: 50
    ...
    try {
      ...
      //   logObject(data)
      // }
      aliasesList = data.Aliases
    } catch (error) {
      throw error
    }

    theFunction.aliasesList = aliasesList
